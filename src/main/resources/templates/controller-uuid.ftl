...
public class ${className} {
    ...
    public ResponseResult get(@PathVariable String uuid) throws BusinessException {
    ...
    public SimpleResponseResult update(@PathVariable String uuid, @RequestBody @ApiParam @Validated({UpdateGroup.class}) ${dtoClassName} param) throws BusinessException {
        // ${dtoClassName} 用来接收请求参数
        String jsonString = JSON.toJSONString(param);
        ${modelClassName} ${modelObjectName} = JSON.parseObject(jsonString, ${modelClassName}.class);
        ${modelObjectName}.set${keyFieldName}(uuid);
    ...
    public ResponseResult delete(@PathVariable String uuid) throws BusinessException {
        // 此处根据实际情况判断row=0是否需要抛出异常
